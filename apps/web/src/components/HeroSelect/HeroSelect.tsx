        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${isMobile ? 3 : 6}, 1fr)`, gap: 12 }}>
            {heroes.map((h) => {
              ...
              return (
                <div key={h.id} role="button" tabIndex={0} onClick={() => setSelectedHero(h.id)} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") setSelectedHero(h.id); }} style={{ cursor: "pointer", padding: "16px 6px 12px", borderRadius: 14, display: "flex", flexDirection: "column", alignItems: "center", background: active ? `linear-gradient(160deg,color-mix(in srgb,${fc} 20%,transparent),rgba(20,26,42,.6))` : "rgba(255,255,255,.03)", border: `1.5px solid ${active ? fc : "rgba(255,255,255,.08)"}`, boxShadow: active ? `0 0 22px color-mix(in srgb,${fc} 40%,transparent)` : "none", transform: active ? "translateY(-3px)" : "none", transition: "all .15s ease" }}>
